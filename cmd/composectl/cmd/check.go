...
func checkApps(ctx context.Context,
		...
		for _, app := range status.Apps {
			...
			err = app.Tree().Walk(func(node *compose.TreeNode, depth int) error {
				...
				if bs.State == compose.BlobFetching {
					fmt.Printf("...%.2f%% (%d)\n", (float64(bs.FetchedSize)/float64(bs.Descriptor.Size))*100, bs.FetchedSize)
				} else {
					fmt.Printf("...%s\n", bs.State.String())
				}
				return nil
	...
	for _, bi := range status.FetchStatus.MissingBlobs {
		if bi.State == compose.BlobFetching {
			checkResult.TotalPullSize += bi.Descriptor.Size - bi.FetchedSize
			checkResult.TotalStoreSize += compose.AlignToBlockSize(bi.Descriptor.Size-bi.FetchedSize, config.BlockSize)
		} else {
