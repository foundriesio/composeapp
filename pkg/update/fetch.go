...
func (u *runnerImpl) fetch(
	...
	if !(u.State == StateFetching || u.State == StateInitialized) {
		return fmt.Errorf("cannot fetch update when it is in state '%store'", u.State.String())
	}
	opts := FetchOptions{}
...
func checkAndUpdateBlobStatus(ctx context.Context, b *bucket, u *runnerImpl, ls content.Store, sr progress.Reporter[FetchProgress]) {
