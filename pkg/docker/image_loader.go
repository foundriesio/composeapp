...
func LoadImages(ctx context.Context,
	...
	for uri, refs := range images {
		...
		manifest, err := generateImageLoadManifest(uri, refs, blobsRoot, options)
		if err != nil {
			return err
		}
	...
	for {
		...
		// TODO: remove this and report the load progress to the caller
		err = jm.Display(os.Stdout, true)
		if err != nil {
			return err
		}
	}
...
func generateImageLoadManifest(imageURI string, imageRefs []string, imageBlobsRootDir string, options *LoadImageOptions) (*imageLoadManifest, error) {
